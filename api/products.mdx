...
Returns the full details of a single active product identified by its numeric ID.

<Note>
  This endpoint only returns products with `status = 1`. Disabled products return `404` rather than `403` to avoid leaking catalog information.
</Note>
...
| `404` | Product does not exist or is disabled. |
